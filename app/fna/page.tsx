...
export default function Page() {
  ...
  // Client table sorting and resizing
  const [clientSort, setClientSort] = useState<ClientSortConfig>({ key: null, direction: 'asc' });
  const [clientColumnWidths, setClientColumnWidths] = useState<Record<string, number>>({
    first_name: 200,
    last_name: 200,
    phone: 180,
    email: 250,
  });
  const [resizingClient, setResizingClient] = useState<{ key: string; startX: number; startWidth: number } | null>(null);
  ...
  // Client table resize effect
  useEffect(() => {
    if (!resizingClient) return;

    const handleMouseMove = (e: MouseEvent) => {
      if (!resizingClient) return;
      const diff = e.clientX - resizingClient.startX;
      const newWidth = Math.max(80, resizingClient.startWidth + diff);
      setClientColumnWidths(prev => ({ ...prev, [resizingClient.key]: newWidth }));
    };

    const handleMouseUp = () => {
      setResizingClient(null);
    };
  ...
  const startClientResize = (e: React.MouseEvent, key: string) => {
    e.preventDefault();
    e.stopPropagation();
    setResizingClient({
      key,
      startX: e.clientX,
      startWidth: clientColumnWidths[key],
    });
  };

  const handleClientSort = (key: keyof ClientRow) => {
    setClientSort(prev => ({
      key,
  ...
  const ClientSortIndicator = ({ columnKey }: { columnKey: string }) => {
    if (clientSort.key !== columnKey) {
      return <span className="ml-1 text-slate-400">⇅</span>;
    }
    return <span className="ml-1">{clientSort.direction === 'asc' ? '↑' : '↓'}</span>;
  };
    ...
    try {
      ...
      if (clientSort.key) {
        results.sort((a, b) => {
          ...
          if (aVal === null || aVal === undefined) return clientSort.direction === 'asc' ? 1 : -1;
          if (bVal === null || bVal === undefined) return clientSort.direction === 'asc' ? -1 : 1;
          
          const comparison = String(aVal).localeCompare(String(bVal));
          return clientSort.direction === 'asc' ? comparison : -comparison;
        });
      }

      setClientRows(results);
    } catch (e: any) {
      setClientRows([]);
        ...
        <div className="rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-1x2 font-bold color-#1E5AA8">{pageTitle}</div>
            ...
            <div className="overflow-auto rounded-lg border border-slate-300">
              <table className="w-full text-sm min-w-[760px] border-collapse border border-slate-300">
                <thead className="bg-slate-100">
                  <tr className="text-left text-xs font-semibold text-slate-700">
                    <th 
                      className="px-4 py-3 border border-slate-300 cursor-pointer hover:bg-slate-200 relative select-none"
                      style={{ width: `${clientColumnWidths.first_name}px`, minWidth: `${clientColumnWidths.first_name}px` }}
                    ...
                    >
                      ...
                      <div
                        ...
                        onMouseDown={(e) => startClientResize(e, 'first_name')}
                        onClick={(e) => e.stopPropagation()}
                      />
                    </th>
                    <th 
                      className="px-4 py-3 border border-slate-300 cursor-pointer hover:bg-slate-200 relative select-none"
                      style={{ width: `${clientColumnWidths.last_name}px`, minWidth: `${clientColumnWidths.last_name}px` }}
                    ...
                    >
                      ...
                      <div
                        className="absolute top-0 right-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-500 bg-slate-400"
                        onMouseDown={(e) => startClientResize(e, 'last_name')}
                        onClick={(e) => e.stopPropagation()}
                      />
                    </th>
                    <th 
                      className="px-4 py-3 border border-slate-300 cursor-pointer hover:bg-slate-200 relative select-none"
                      style={{ width: `${clientColumnWidths.phone}px`, minWidth: `${clientColumnWidths.phone}px` }}
                      onClick={() => handleClientSort('phone')}
                    >
                      <div className="flex items-center justify-between">
                        <span>Phone</span>
                        <ClientSortIndicator columnKey="phone" />
                      </div>
                      <div
                        className="absolute top-0 right-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-500 bg-slate-400"
                        onMouseDown={(e) => startClientResize(e, 'phone')}
                        onClick={(e) => e.stopPropagation()}
                      />
                    </th>
                    <th 
                      className="px-4 py-3 border border-slate-300 cursor-pointer hover:bg-slate-200 relative select-none"
                      style={{ width: `${clientColumnWidths.email}px`, minWidth: `${clientColumnWidths.email}px` }}
                      onClick={() => handleClientSort('email')}
                    >
                      <div className="flex items-center justify-between">
                        <span>Email</span>
                        <ClientSortIndicator columnKey="email" />
                      </div>
                      <div
                        className="absolute top-0 right-0 bottom-0 w-1 cursor-col-resize hover:bg-blue-500 bg-slate-400"
                        onMouseDown={(e) => startClientResize(e, 'email')}
                        onClick={(e) => e.stopPropagation()}
                      />
                    </th>
                  </tr>
                  ...
                  {clientLoading ? (
                    ...
                    clientRows.map((c) => {
                      ...
                      return (
                        ...
                        >
                          <td className="px-4 py-3 border border-slate-300 font-semibold text-slate-900 truncate" style={{ width: `${clientColumnWidths.first_name}px` }}>
                            {c.first_name}
                          </td>
                          <td className="px-4 py-3 border border-slate-300 text-slate-900 truncate" style={{ width: `${clientColumnWidths.last_name}px` }}>{c.last_name}</td>
                          <td className="px-4 py-3 border border-slate-300 text-slate-700 truncate" style={{ width: `${clientColumnWidths.phone}px` }}>{c.phone}</td>
                          <td className="px-4 py-3 border border-slate-300 text-slate-700 truncate" style={{ width: `${clientColumnWidths.email}px` }}>{c.email}</td>
                        </tr>
                      );
                    })
                  )}
